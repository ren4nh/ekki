    ...
    return (
      ...
      >
        <Grid item className={classes.grid} sm={10} xs={6}>
            ...
            <CardContent>
              <form onSubmit={handleSubmit(this.onSubmit)}>
                ...
                <Typography gutterBottom>
                  {" "}
                  <strong> Saldo atual: </strong> R$ 0,00{" "}
                </Typography>
                {usingCreditCard && (
                  <Typography gutterBottom>
                    {" "}
                    <strong>
                      {" "}
                      Será utilizado o cartão de crédito para completar a
                      transação{" "}
                    </strong>{" "}
                  </Typography>
                )}
          ...
          { this.state.showModal &&  <PasswordModal toogle={this.toggleModal} callback={this.submitForm} /> }
        </Grid>
        <Grid item className={classes.grid} sm={10} xs={6}>
...
const validate = (values, { user }) => {
  ...
  if (!values.destination) {
    ...
    !/^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,4}$/i.test(values.destination)
  ) {
